...
class DatabaseOps():
    ...
    async def GetUserByusername(self, db: AsyncSession, user: str):
        result = await db.execute(select(User).where(User.username == user))
        return result.scalar_one_or_none()
    
    async def GetUserFileByUserid(self, db: AsyncSession, user_id: str):
    ...
    async def remove_file(self, db: AsyncSession, user_id: str, file_id: int):
    ...
    async def add(self, db: AsyncSession, user: User):
        db.add(user)
        await db.commit()
    ...
    async def add_file(self, db: AsyncSession, file: FileModel):
        db.add(file)
        await db.commit()
